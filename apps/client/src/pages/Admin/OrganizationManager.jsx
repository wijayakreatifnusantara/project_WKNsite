  ...
  const handleDeleteDept = async (orgId, deptId) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        showToast('error', data.detail || 'Gagal menghapus departemen');
      }
    } catch (err) {
      showToast('error', 'Koneksi ke server gagal');
    }
                    ...
                    <div className="border-t border-slate-100 bg-slate-50/50 p-6 space-y-4 animate-in slide-in-from-top-4 duration-300">
                      ...
                      {orgDepts.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                          {orgDepts.map(dept => (
                            <div key={dept.id} className="bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between gap-3 shadow-xs hover:border-slate-300 transition-all">
                              <div className="space-y-1">
