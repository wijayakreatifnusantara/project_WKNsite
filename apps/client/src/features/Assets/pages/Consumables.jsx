      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex justify-between items-end">
          <div>
            ...
            <p className="text-slate-400 text-xs mt-1 font-black uppercase tracking-[0.3em] opacity-70">Supply Chain & Inventory Management</p>
          </div>
          <div className="flex gap-4">
            <Button className="h-12 px-6 rounded-2xl bg-white shadow-sm text-slate-600 font-black text-xs uppercase tracking-widest hover:text-[#E31E24] hover:shadow-none transition-all flex gap-3 items-center border-2 border-white">
              <IconQrcode size={16} />
              Scan QR Checkout
            </Button>
...
<table className="w-full text-left border-collapse min-w-max">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-wider">Item Name</th>
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-wider">Current Stock</th>
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-wider">Minimum Threshold</th>
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-wider">Status</th>
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-wider">Action</th>
              </tr>
            </thead>
            <tbody>
              {inventory.map(item => (
                <tr key={item.id} className="border-b border-white/30 hover:bg-white/30 transition-colors group">
                  ...
                  <td className="px-8 py-5">
                    <span className="text-sm font-black text-slate-800">{item.stock} <span className="text-xs text-slate-400 uppercase">{item.unit}</span></span>
                  </td>
                  <td className="px-8 py-5">
                  ...
                  <td className="px-8 py-5">
                    <Button variant="ghost" className="h-10 px-4 rounded-xl text-xs font-black uppercase text-[#E31E24] hover:bg-rose-50 flex gap-2">
                      <IconPlus size={14} />
                      Request
                    </Button>
                  </td>
