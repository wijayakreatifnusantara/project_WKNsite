...
const LeaveManagementHub = () => {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 bg-transparent custom-scrollbar animate-fade-in text-xs">
      <div className="max-w-[1600px] mx-auto space-y-4">
        ...
        <div className="flex items-center justify-between bg-transparent p-2 px-5 rounded-2xl border border-slate-200 shadow-sm ">
          <div className="flex items-center gap-4">
             <button 
               ...
               className="h-8 w-8 rounded-lg bg-white shadow-sm border-none shadow-sm flex items-center justify-center text-slate-400 hover:text-ios-primary hover:border-ios-primary/20 hover:bg-transparent transition-all active:scale-95"
             >
               <IconArrowLeft size={16} />
             </button>
             <div className="h-8 w-8 bg-[#E31E24]/10 rounded-lg flex items-center justify-center text-ios-primary">
                <IconClipboardCheck size={18} />
             </div>
             <div>
                <h2 className="text-sm font-bold text-slate-800 uppercase tracking-tight leading-none">
                  Leave <span className="text-ios-primary">Management Hub</span>
                </h2>
                <div className="flex items-center gap-2 mt-1">
                   <div className="h-1 w-1 bg-[#E31E24] rounded-full animate-pulse"></div>
                   <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none">Workflow & Balances Sync Active</span>
                </div>
             </div>
          </div>
          ...
          <div className="flex items-center gap-3">
              <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-xl border border-slate-200">
                  {['Pending', 'Approved', 'Rejected', 'All'].map((s) => (
                    <button 
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-widest transition-all ${filterStatus === s ? 'bg-transparent shadow-sm text-ios-primary' : 'text-slate-400 hover:text-slate-600'}`}
                    >
                      {s}
                    </button>
                  ))}
              ...
              <Button 
                ...
                className="h-8 px-4 rounded-lg bg-[#E31E24] text-white font-bold text-[11px] uppercase tracking-widest shadow-sm hover:bg-[#C1181E] flex gap-2 items-center"
              >
                <IconPlus size={14} />
                New Request
              </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
           <StatCard 
             data-tooltip="Pending Reviews" 
             value={requests.filter(r => r.status?.startsWith('Pending')).length} 
             icon={<IconClock size={16} />} 
             color="amber"
           />
           <StatCard 
             data-tooltip="Approved Requests" 
             value={requests.filter(r => r.status === 'Approved').length} 
             icon={<IconCheck size={16} />} 
             color="emerald"
           />
           <StatCard 
             data-tooltip="Total On Leave (Today)" 
           ...
           />
           <StatCard 
             data-tooltip="Total Requests Processed" 
            ...
            <table className="w-full text-left border-collapse min-w-max">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-200">
                  <th className="px-5 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Employee</th>
                  <th className="px-5 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Type & Duration</th>
                  <th className="px-5 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Period</th>
                  <th className="px-5 py-3 text-center text-[11px] font-bold text-slate-400 uppercase tracking-widest">Status</th>
                  <th className="px-5 py-3 text-right text-[11px] font-bold text-slate-400 uppercase tracking-widest">Authorization</th>
                </tr>
              </thead>
                      ...
                      <td className="px-5 py-2">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] font-bold text-slate-400 tracking-widest uppercase">{row.employees?.division_name || row.employee_id}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-2">
                        <div className="flex flex-col">
                           ...
                           <span className="text-[11px] font-bold text-ios-primary uppercase mt-0.5">{row.days_count} Working Days</span>
                        </div>
                      </td>
                      <td className="px-5 py-2">
                          ...
                          {row.status?.startsWith('Pending') && isAdmin() ? (
                            <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all">
                              <button 
                                ...
                                className="h-7 px-3 rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all text-[11px] font-bold uppercase tracking-widest flex items-center gap-1.5 shadow-sm"
                              >
                                <IconCheck size={12} />
                                Approve
                              </button>
                              <button 
                                ...
                                className="h-7 px-3 rounded-lg bg-rose-50 text-rose-600 border border-rose-100 hover:bg-rose-600 hover:text-white transition-all text-[11px] font-bold uppercase tracking-widest flex items-center gap-1.5 shadow-sm"
                              >
                                <IconX size={12} />
                                Reject
                              </button>
...
const StatCard = ({ title, value, unit = "", icon, color }) => {
  ...
  return (
    <Card className="bg-transparent border border-slate-200 shadow-sm p-3 flex items-center gap-3 transition-all hover:translate-y-[-2px] cursor-pointer">
      ...
      <div>
        ...
        <div className="flex items-baseline gap-1">
          ...
          {unit && <span className="text-[11px] font-bold text-slate-400 uppercase">{unit}</span>}
        </div>
      </div>
    </Card>
  );
