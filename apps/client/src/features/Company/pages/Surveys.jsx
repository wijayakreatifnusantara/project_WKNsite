        ...
        <header className="flex justify-between items-end">
          <div>
            <h2 className="text-2xl font-bold text-slate-800 font-outfit tracking-tight uppercase">
              Voice of <span className="text-ios-primary">Personnel</span>
            </h2>
            <p className="text-slate-400 text-xs mt-1 font-bold uppercase tracking-widest opacity-70">Engagement Surveys & Feedback Loop</p>
          </div>
          <Button variant="outline" className="h-12 px-6 rounded-2xl bg-white shadow-sm border-white border-2 text-slate-600 font-bold text-xs uppercase tracking-widest hover:shadow-none transition-all flex gap-3 items-center">
        ...
        <Card className="bg-white border-white border-[4px] shadow-sm rounded-[2.5rem] p-10 flex items-center justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-xl font-bold text-slate-800 uppercase tracking-tight">WKN Engagement Score</h3>
            </div>
          ...
          <div className="h-32 w-32 rounded-full border-[10px] border-[#f0f2f5] shadow-sm flex flex-col items-center justify-center bg-white/20">
             <span className="text-3xl font-bold text-emerald-500 font-outfit">8.8</span>
             <span className="text-[11px] font-semibold text-slate-400 uppercase">Excellent</span>
          </div>
        </Card>

        {/* Active Surveys */}
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider px-1">Available Surveys</h3>
            ...
            {activeSurveys.map(sv => (
              <Card key={sv.id} className="border-white border-[3px] shadow-sm bg-white rounded-2xl p-6 flex items-center justify-between group">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-slate-800 uppercase tracking-tight">{sv.title}</h4>
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{sv.questions} Questions • {sv.duration} ESTIMATED</p>
                  </div>
                </div>
                <div className="flex items-center gap-12">
                   <div className="text-right flex flex-col items-end">
                      <span className={`px-3 py-1 rounded-lg text-[11px] font-semibold uppercase tracking-widest border mb-1
                        ${sv.status === 'Required' ? 'bg-rose-50 text-rose-500 border-rose-100' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>
                        {sv.status}
                      </span>
                      <div className="flex items-center gap-2 text-slate-400">
                        <IconClock size={12} />
                        <span className="text-[11px] font-semibold uppercase">Expires: {sv.deadline}</span>
                      </div>
                   </div>
                   <Button className="h-12 px-6 rounded-xl bg-ios-primary text-white font-bold text-xs uppercase tracking-widest shadow-sm hover:bg-ios-primary/90 transition-all flex gap-3 items-center">
                     Take Survey
