      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex justify-between items-end">
          <div>
            ...
            <p className="text-slate-400 text-xs mt-1 font-black uppercase tracking-[0.3em] opacity-70">Corporate SOPs & Knowledge Base</p>
          </div>
          <div className="h-12 w-64 px-4 bg-white shadow-sm border-white border-2 rounded-2xl flex items-center gap-3">
            ...
            <input 
              ...
              className="bg-transparent border-none text-xs font-black uppercase tracking-widest text-slate-700 w-full focus:outline-none focus:border-[#E31E24] focus:ring-2 focus:ring-[#E31E24]/20 transition-all"
            />
          </div>
        </header>
        ...
        <div className="space-y-4">
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider px-1">Recently Updated Articles</h3>
          <div className="grid grid-cols-1 gap-4">
            {articles.map(art => (
              <Card key={art.id} className="border-white border-[3px] shadow-sm bg-white rounded-xl p-6 flex items-center justify-between group hover:scale-[1.005] transition-all cursor-pointer relative overflow-hidden">
                {art.id === 1 && (
                  <div className="absolute top-0 left-0 h-1 w-full bg-[#E31E24] shadow-sm"></div>
                )}
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      {art.id === 1 && (
                        <span className="px-2 py-0.5 rounded bg-rose-50 text-rose-500 text-[11px] font-black uppercase tracking-widest border border-rose-100">Mandatory</span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-8">
                  <div className="text-right flex items-center gap-2 text-slate-400">
                    <IconClock size={12} />
                    <span className="text-xs font-black uppercase">{art.lastUpdate}</span>
                  </div>
                  {art.id === 1 ? (
                    <Button className="h-10 px-6 rounded-xl bg-[#E31E24] text-white text-[11px] font-black uppercase tracking-widest shadow-sm hover:bg-[#C1181E] transition-all">
                       Confirm Read
                    </Button>
                  ) : (
                    <div className="flex items-center gap-2 text-emerald-500">
                       <IconCircleCheck size={16} />
                       <span className="text-[11px] font-black uppercase">Read</span>
                    </div>
                  )}
...
const WikiLink = ({ label, count, icon, color }) => (
  <Card className="border-white border-[2px] shadow-sm bg-white rounded-xl p-6 group cursor-pointer hover:shadow-none transition-all">
    ...
    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">{count}</p>
  </Card>
);
