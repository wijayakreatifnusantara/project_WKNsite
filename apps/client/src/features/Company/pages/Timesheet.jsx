      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex justify-between items-end">
          <div>
            <h2 className="text-2xl font-bold text-slate-800 font-outfit tracking-tight uppercase">
              Project <span className="text-ios-primary">Timesheets</span>
            </h2>
            <p className="text-slate-400 text-xs mt-1 font-bold uppercase tracking-widest opacity-70">Billable Hours & Resource Allocation</p>
          </div>
          <div className="flex gap-4">
            <div className="flex bg-white shadow-sm rounded-2xl p-1">
              <button 
                onClick={() => setFilter('pending')}
                className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${filter === 'pending' ? 'bg-ios-primary text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
              >
                Need Approval
              </button>
              <button 
                onClick={() => setFilter('all')}
                className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${filter === 'all' ? 'bg-ios-primary text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
              >
                All Logs
              </button>
            </div>
        ...
        <Card className="bg-white border-white border-[4px] shadow-sm rounded-[2.5rem] p-8 flex flex-col gap-6">
           <div className="flex justify-between items-center px-4">
             <h3 className="text-lg font-bold text-slate-800 uppercase tracking-tight">Weekly Summary</h3>
             <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{dayjs().startOf('week').format('MMM DD')} - {dayjs().endOf('week').format('MMM DD')}</span>
           </div>
             ...
             {weeklyData.map((h, i) => (
               <div key={i} className="flex-1 flex flex-col items-center gap-3">
                 <div className="w-full bg-white shadow-sm rounded-xl h-32 relative overflow-hidden flex items-end">
                   <div className="w-full bg-ios-primary transition-all duration-1000" style={{ height: `${h}%` }}></div>
                 </div>
                 <span className="text-[11px] font-semibold text-slate-400 uppercase">{['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][i]}</span>
               </div>
             ))}
           </div>
        </Card>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-1">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">{filter === 'pending' ? 'Pending Approval' : 'Timesheet Logs'}</h3>
...
const TimesheetLog = ({ log, onApprove, onReject }) => (
  <Card className="border-white border-[3px] shadow-sm bg-white rounded-xl p-6 flex flex-col md:flex-row md:items-center justify-between group gap-4">
    <div className="flex items-center gap-6">
      ...
      <div>
        <h4 className="text-sm font-bold text-slate-800 uppercase tracking-tight">{log.employees?.name || 'Unknown Employee'}</h4>
        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">
          <span className="text-ios-primary">{log.project_name}</span> • {dayjs(log.date).format('DD MMM YYYY')}
        </p>
    ...
    <div className="flex items-center gap-8 md:gap-12 justify-between md:justify-end">
      <div className="text-right">
        <p className="text-lg font-bold text-slate-700 font-outfit">{log.duration_hours}h</p>
