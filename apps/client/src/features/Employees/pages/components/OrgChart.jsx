          ...
          <div className="flex gap-12 items-start">
            {Object.entries(deptTree).map(([dept, staff]) => (
              <div key={dept} className="flex flex-col items-center">
                {/* Dept Node */}
                <div className="bg-transparent border border-slate-200/80 shadow-sm rounded-xl p-4 w-[210px] relative">
                  <div className="flex items-center gap-3">
                  ...
                  {staff.filter(e => !searchTerm || e["EMPLOYEE NAME"].toLowerCase().includes(searchTerm.toLowerCase())).slice(0, 15).map((emp) => (
                    ...
                    >
                      ...
                      <div className="flex-1 overflow-hidden text-left leading-none">
                        <h4 className="text-xs font-bold text-slate-800 uppercase truncate mb-1">{emp["EMPLOYEE NAME"]}</h4>
                        <p className="text-[11px] font-semibold text-slate-450 uppercase truncate">{emp["Job Position *"]}</p>
                      </div>
                    </div>
                  ))}
                  {staff.length > 15 && (
