...
const AuditTrail = ({ isOpen, onClose, employee }) => {
  ...
  return (
    <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm  animate-in fade-in duration-300">
      <div 
          ...
          {logs.map((log, index) => (
            <div key={log.id} className="relative flex gap-12 group animate-in slide-in-from-left-4 duration-500" style={{ animationDelay: `${index * 100}ms` }}>
              ...
              <div className="relative z-10 pt-1">
                <div className={`h-8 w-8 rounded-xl bg-transparent shadow-sm flex items-center justify-center ${log.color} border-2 border-white ring-4 ring-[#f0f2f5]`}>
                  {log.icon}
                </div>
              </div>
