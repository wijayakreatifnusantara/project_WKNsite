          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              ...
              {shifts.length === 0 ? (
                ...
                <div className="overflow-hidden bg-transparent rounded-2xl border border-slate-200/60 shadow-xs">
...
<table className="w-full text-left border-collapse min-w-max">
                    ...
                    <tbody className="text-xs text-slate-600 divide-y divide-white/50">
                      {holidays.map(hol => (
                        <tr key={hol.id} className="transition-colors hover:bg-slate-50 transition-colors cursor-pointer">
                          <td className="px-5 py-3 font-semibold text-slate-700">
                            {hol.start_date} {hol.end_date && hol.end_date !== hol.start_date ? ` s/d ${hol.end_date}` : ''}
                          </td>
                          <td className="px-5 py-3 font-bold text-slate-800">
                            {hol.name}
                            {hol.description && <span className="block font-normal text-slate-400 text-xs mt-0.5">{hol.description}</span>}
                          </td>
                          <td className="px-5 py-3">
                            <span className="px-2 py-1 rounded bg-indigo-50 text-indigo-600 border border-indigo-100 text-xs font-bold tracking-wider">
                              {hol.type}
                            </span>
                          </td>
                          <td className="px-5 py-3 text-right">
            ...
            <form onSubmit={handleSubmitShift} className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full h-10 px-3 rounded-xl bg-white shadow-sm border-none text-slate-800 font-bold text-xs"
                  />
                </div>
                <div className="space-y-1">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Aktifkan jika jam pulang melewati tengah malam (hari berikutnya).</p>
                </div>
                <button
                  type="button"
