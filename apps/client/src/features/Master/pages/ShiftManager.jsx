    ...
    <div className="flex-1 flex flex-col min-h-0 bg-[#f8fafc] font-inter animate-fade-in">
          ...
          <div className="flex p-1 bg-slate-100/80 rounded-xl border border-white/50/60">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all duration-300 ${
                activeTab === 'shifts' ? 'bg-transparent shadow-neu border border-white/50/50 text-[#E31E24] scale-100' : 'text-slate-500 hover:text-slate-800'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all duration-300 ${
                activeTab === 'holidays' ? 'bg-transparent shadow-neu border border-white/50/50 text-[#E31E24] scale-100' : 'text-slate-500 hover:text-slate-800'
              }`}
            >
      ...
      {message.text && (
        <div className="px-4 pt-4 w-full mx-auto">
          <div className={`p-3 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4 shadow-neu ${message.type === 'success' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
        ...
        <div className="w-full mx-auto">
          {isLoading ? (
            ...
            <div className="space-y-4">
              ...
              {shifts.length === 0 ? (
                <div className="bg-transparent rounded-2xl border border-white/50 p-12 text-center shadow-xs">
                ...
                <div className="overflow-hidden bg-transparent rounded-2xl border border-white/50/60 shadow-xs">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-white/50/60">
                    ...
                    <tbody className="text-xs text-slate-600 divide-y divide-white/50">
                      {shifts.map(shift => (
                        <tr key={shift.id} className="hover:shadow-neu-inset/50 transition-colors group">
                          ...
                          <td className="px-5 py-3">
                            <span className={`text-[8px] font-black px-2 py-1 rounded uppercase tracking-wider border ${shift.is_active ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-slate-100 text-slate-500 border-white/50'}`}>
              ...
              {holidays.length === 0 ? (
                ...
                <div className="overflow-hidden bg-transparent rounded-2xl border border-white/50/60 shadow-xs">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="text-xs text-slate-600 divide-y divide-white/50">
                      {holidays.map(hol => (
                        <tr key={hol.id} className="hover:shadow-neu-inset/50 transition-colors">
                          <td className="px-5 py-3 font-semibold text-slate-700">
      ...
      {isShiftModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-transparent rounded-3xl p-6 w-full max-w-lg shadow-neu space-y-4">
            ...
            <form onSubmit={handleSubmitShift} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full h-10 px-3 rounded-xl bg-[#f0f2f5] shadow-neu-inset border-none text-slate-800 font-bold text-xs focus:ring-1 focus:ring-[#E31E24]/30"
                  />
                </div>
                <div className="space-y-1">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full h-10 px-3 rounded-xl bg-[#f0f2f5] shadow-neu-inset border-none text-slate-800 font-bold text-xs"
                  />
                </div>
                <div className="space-y-1">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-white/50 flex items-center justify-between">
                <div>
              ...
              <div className="pt-2 flex justify-end gap-2">
                <button type="button" onClick={() => setIsShiftModalOpen(false)} className="h-10 px-5 bg-transparent text-slate-600 font-black text-[9px] uppercase tracking-widest rounded-xl hover:shadow-neu-inset border border-white/50 transition-colors">
                  Batal
                </button>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-transparent rounded-3xl p-6 w-full max-w-md shadow-neu space-y-4">
            ...
            <form onSubmit={handleSubmitHoliday} className="space-y-4">
              ...
              <div className="pt-2 flex justify-end gap-2">
                <button type="button" onClick={() => setIsHolidayModalOpen(false)} className="h-10 px-5 bg-transparent text-slate-600 font-black text-[9px] uppercase tracking-widest rounded-xl hover:shadow-neu-inset border border-white/50 transition-colors">
                  Batal
                </button>
                <button type="submit" disabled={isSubmitting} className="h-10 px-5 bg-[#E31E24] text-white font-black text-[9px] uppercase tracking-widest rounded-xl hover:bg-[#C1181E] transition-colors flex items-center gap-2">
