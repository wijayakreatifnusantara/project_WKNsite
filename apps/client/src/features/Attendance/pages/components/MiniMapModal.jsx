...
const MiniMapModal = ({ isOpen, onClose, data }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm animate-fade-in">
      <Card className="w-full max-w-md overflow-hidden bg-transparent shadow-neu rounded-2xl animate-scale-up border-0 ring-1 ring-slate-200">
        <div className="flex items-center justify-between px-5 py-4 bg-slate-50/80 backdrop-blur-md border-b border-white/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-medium text-slate-500">{target_name || 'Unknown Location'}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-5 space-y-4">
          {mapUrl ? (
            <div className="relative w-full h-64 overflow-hidden border border-white/50 rounded-xl bg-slate-100">
              <iframe 
                src={mapUrl}
                width="100%" 
                height="100%" 
            ...
            <div className="flex flex-col items-center justify-center w-full h-64 text-center border border-dashed rounded-xl border-white/50 bg-slate-50">
          ...
          <div className="grid grid-cols-2 gap-3">
             <div className="p-3 bg-[#f0f2f5] shadow-neu-inset border-none rounded-xl">
                <p className="text-[8px] font-black tracking-widest text-slate-400 uppercase mb-1">Target Location</p>
                <p className="text-xs font-bold text-slate-700 truncate">{target_name || '-'}</p>
             </div>
             <div className="p-3 bg-[#f0f2f5] shadow-neu-inset border-none rounded-xl">
                <p className="text-[8px] font-black tracking-widest text-slate-400 uppercase mb-1">Distance Variance</p>
                <p className="text-xs font-bold text-slate-700">
                   {distance_meters != null ? `${distance_meters} meters` : '-'}
                </p>
             </div>
             <div className="p-3 bg-[#f0f2f5] shadow-neu-inset border-none rounded-xl col-span-2">
                <p className="text-[8px] font-black tracking-widest text-slate-400 uppercase mb-1">Coordinates</p>
