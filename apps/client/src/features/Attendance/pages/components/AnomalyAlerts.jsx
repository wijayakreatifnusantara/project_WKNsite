        ...
        {alerts.map(alert => (
          <div key={alert.id} className="p-2.5 rounded-xl border border-rose-100 bg-rose-50/50 flex gap-3 group hover:bg-rose-50 transition-colors cursor-pointer">
            ...
            <div className="min-w-0 flex-1">
              <h4 className="text-[10px] font-black text-slate-800 uppercase tracking-tight truncate">{alert.message}</h4>
              <div className="flex items-center justify-between mt-1">
                <span className="text-[8px] font-bold text-slate-500 uppercase">{alert.user}</span>
                <span className="text-[8px] font-black text-rose-400">{alert.time}</span>
              </div>
            </div>
          </div>
        ))}
