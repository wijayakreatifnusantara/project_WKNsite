    ...
    <div className="flex-1 overflow-y-auto p-8 bg-transparent custom-scrollbar animate-fade-in">
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex justify-between items-end">
          <div>
            <h2 className="text-2xl font-bold text-slate-800 font-outfit tracking-tight uppercase">
              WKN<span className="text-ios-primary"> Academy</span>
            </h2>
            <p className="text-slate-400 text-xs mt-1 font-bold uppercase tracking-widest opacity-70">Knowledge Hub & Professional Development</p>
          </div>
        ...
        <div className="space-y-6">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider px-1">Active Learning Paths</h3>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {courses.map(course => (
              <Card key={course.id} className="border-white border-[3px] shadow-sm bg-white rounded-2xl p-6 flex items-center gap-6 group hover:scale-[1.01] transition-all">
                <div className="h-20 w-20 bg-white rounded-xl shadow-sm flex items-center justify-center text-ios-primary">
                  <IconSchool size={32} />
                </div>
                <div className="flex-1 space-y-2">
                  <div className="flex justify-between items-start">
                    <h4 className="text-sm font-bold text-slate-800 uppercase tracking-tight">{course.title}</h4>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 h-2 bg-white shadow-sm rounded-full overflow-hidden">
                      <div className="h-full bg-ios-primary transition-all duration-1000" style={{ width: `${course.progress}%` }}></div>
                    </div>
                  ...
                  <div className="flex justify-between items-center pt-2">
                    ...
                    <Button variant="ghost" className="h-8 px-4 rounded-xl text-xs font-bold uppercase text-ios-primary hover:bg-rose-50 flex gap-2">
